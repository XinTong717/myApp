...
export default defineConfig<'webpack5'>(async (merge, { mode }) => {
  ...
  if (isProductionBuild && !process.env.TARO_APP_CLOUD_ENV) {
    throw new Error('[cloud] Missing TARO_APP_CLOUD_ENV for production build. Refusing to fallback to the dev cloud environment.')
  }
  ...
  const baseConfig: UserConfigExport<'webpack5'> = {
    ...
    copy: {
      patterns: [
        { from: 'src/assets/', to: 'dist/assets/', ignore: ['*.ts'] },
      ],
      options: {
      }
    },
