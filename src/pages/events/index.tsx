...
function FilterChip(props: { label: string; active: boolean; onClick: () => void }) {
  return <View onClick={props.onClick} style={{ padding: `6px ${space(3)}`, borderRadius: '999px', marginRight: space(2), marginBottom: space(2), backgroundColor: props.active ? palette.action : palette.tag, border: `1px solid ${props.active ? palette.action : palette.line}` }}><Text style={{ fontSize: '12px', color: props.active ? '#FFF' : palette.tagText }}>{props.label}</Text></View>
}
