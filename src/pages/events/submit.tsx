...
export default function SubmitEventPage() {
  ...
  const handleSubmit = async () => {
    ...
    const confirm = await Taro.showModal({
      title: '提交活动',
      content: '提交后会进入人工审核队列，审核通过后才会出现在活动列表中。你可以发布自己组织的活动，也可以推荐你认为值得被看见的公开活动。',
      confirmText: '确认提交',
      cancelText: '再看看',
    })
    ...
    try {
      setSubmitting(true)
      const finalFee = fee === '付费' && feeDetail.trim() ? `付费：${feeDetail.trim()}` : fee
  ...
  return (
    <View style={{ minHeight: '100vh', backgroundColor: palette.bg, padding: '16px', boxSizing: 'border-box' }}>
      <View style={{
        ...
        <View style={{ marginTop: '6px' }}>
          <Text style={{ fontSize: '13px', color: palette.subtext, lineHeight: '20px' }}>
            你可以提交自己组织的活动，也可以推荐你认为值得被看见的公开活动。请优先填写公开链接和你有权公开的报名方式。
          </Text>
        </View>
      </View>
      ...
      <View style={{ backgroundColor: palette.card, borderRadius: '20px', padding: '16px', border: `1px solid ${palette.line}` }}>
        ...
        <PillSelect options={FEE_OPTIONS} selected={fee} onChange={(value) => {
          setFee(value)
        ...
        <View style={{ backgroundColor: '#FFFDF9', borderRadius: '14px', padding: '10px 12px', marginBottom: '12px', border: `1px solid ${palette.line}` }}>
          <Textarea value={signupNote} placeholder='例如：公众号报名、表单报名、你愿意公开的微信号等' maxlength={200} onInput={(e) => setSignupNote(e.detail.value)} style={{ width: '100%', minHeight: '60px', fontSize: '14px', color: palette.text }} />
        </View>
