  ...
  return (
    <View onClick={onClose} style={{ position: 'fixed', left: '0', right: '0', top: '0', bottom: '0', backgroundColor: exploreTheme.overlay, display: 'flex', alignItems: 'flex-end', zIndex: 30 }}>
      <View onClick={(e: any) => e?.stopPropagation?.()} style={sheetStyle}>
        ...
        <View onClick={onPrimaryAction} style={user.isSelf ? ghostButtonStyle : primaryButtonStyle}>
          <Text style={{ fontSize: '15px', color: user.isSelf ? exploreTheme.tagText : '#FFF', fontWeight: 'bold' }}>
            {user.isSelf ? '去看我的资料' : hasProfile ? '发起联络' : '去填写资料'}
          </Text>
        </View>
