  ...
  return (
    <>
      ...
      <View onClick={preview || interestLoading ? undefined : onToggleInterest} style={{ backgroundColor: hasInterested ? palette.surfaceSoft : preview ? palette.disabledBg : palette.brand, borderRadius: radius.md, padding: space(3), textAlign: 'center', marginBottom: space(3) }}>
        <Text style={{ ...typography.button, color: hasInterested || preview ? palette.subtext : palette.card }}>{preview ? '完整详情加载后可标记感兴趣' : interestLoading ? '处理中...' : hasInterested ? '已感兴趣，再点一次取消' : '我感兴趣'}</Text>
      </View>
      ...
      <AppCard radius={radius.lg}><View style={{ marginBottom: space(2) }}><Text style={{ ...typography.cardTitle, color: palette.text }}>详细介绍</Text></View><Text style={{ ...typography.body, color: palette.text, whiteSpace: 'pre-wrap' }}>{event.description || '暂无详细介绍'}</Text></AppCard>
    </>
  )
}
